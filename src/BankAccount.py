...
class AceBank(object):
    def __init__(self, log=None, account_details=None):
    ...
    def create_account(self, account_number, balance, customer_id):
        ...
        try:
            ...
            self.log.warning("We expect account should not exist so cont")
    ...
    def deposit_fund(self, account_number, amount, currency, customer_id):
        ...
        self.account_details[customer_id][account_number]["balance"] = (
            self.account_details[customer_id][account_number]["balance"] + amount
        )

    def withdraw_funds(self, account_number, amount, currency, customer_id):
        self.validate_variable(
            account_number=account_number, amount=amount, currency=currency
        )
        self.validate_account(account_number, customer_id)
        amount = self.currency_converter(amount, currency)
        self.account_details[customer_id][account_number]["balance"] = round(
            self.account_details[customer_id][account_number]["balance"] - amount, 2
        )
    ...
    def transfer_funds(
        ...
        self.validate_balance(
            self.account_details[senders_customer_id][senders_account_number][
                "balance"
            ],
            amount,
        )
        ...
        self.account_details[receivers_customer_id][receivers_account_number][
            "balance"
        ] = (
            self.account_details[receivers_customer_id][receivers_account_number][
                "balance"
    ...
    def validate_balance(account_balance, amount):
        if account_balance < amount:
            raise ValueError(
                f"Account balance of {account_balance} is currently lesser than {amount}"
            )

    @staticmethod
    def validate_variable(
        account_number=None, amount=None, currency=None, outgoing_account_number=None
    ):
        ...
        if amount and not isinstance(amount, float):
            raise ValueError("Amount must be of type float")
        if currency and currency not in ACCEPTED_CURRENCY:
            raise ValueError(
                f"Currency {currency} is not recognized. Should be one of {ACCEPTED_CURRENCY}"
            )
        if outgoing_account_number and not isinstance(outgoing_account_number, str):
            raise ValueError(f"Account number {outgoing_account_number} must be string")
